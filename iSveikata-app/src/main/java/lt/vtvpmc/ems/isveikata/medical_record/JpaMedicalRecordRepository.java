...
//	SELECT DATE, count(*) as visits,  sum(DURATION) as duration   FROM MEDICAL_RECORD join APPOINTMENT on MEDICAL_RECORD.APPOINTMENT_ID = APPOINTMENT.ID  where MEDICAL_RECORD.DOCTOR_ID = 594
//	and APPOINTMENT.DATE > '2000-01-01' and APPOINTMENT.DATE < '2016-01-01'  group by DATE order by DATE
	@Query(value = "SELECT DATE, count(*) as visits, "+
			"sum(DURATION) as duration   FROM MEDICAL_RECORD " +
			"join APPOINTMENT on MEDICAL_RECORD.APPOINTMENT_ID = APPOINTMENT.ID " +
			"where MEDICAL_RECORD.DOCTOR_ID = ?1 " +
			"and APPOINTMENT.DATE > ?2 " +
			"and APPOINTMENT.DATE < ?3 " +
			"group by DATE order by DATE",
			nativeQuery = true)
	List getDoctorWorkDaysStatistic(Long doctorId, String dateFrom, String dateTill);
